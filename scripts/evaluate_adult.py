#!/usr/bin/env python3
"""Evaluation script for CF-TCVAE on Adult Income dataset.

This script loads a trained CF-TCVAE model and evaluates it on the Adult test set
using all 8 metrics from Table 1 of the paper.

Usage:
    python scripts/evaluate_adult.py --checkpoint checkpoints/adult_best.pth
    python scripts/evaluate_adult.py --checkpoint checkpoints/adult_best.pth --num_samples 1000
    python scripts/evaluate_adult.py --checkpoint checkpoints/adult_best.pth --save_visualizations
"""
...
from cf_tcvae.config import AdultConfig
from cf_tcvae.datasets import create_adult_loaders
from cf_tcvae.classifiers import load_adult_classifier
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Evaluate CF-TCVAE on Adult Income dataset')
    ...
    parser.add_argument('--save_statistics', action='store_true',
                        help='Save detailed statistics about counterfactuals')
...
def load_model_from_checkpoint(checkpoint_path: str, device: str, logger: logging.Logger) -> CFTCVAE:
    ...
    model = CFTCVAE(
        ...
        input_dim=getattr(config, 'input_dim', 108),
    ).to(device)
...
def generate_counterfactuals(
    ...
    num_classes = 2  # Adult has 2 classes (<=50K, >50K)
    ...
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(tqdm(dataloader, desc="Generating CFs")):
            ...
            if target_class is not None:
                ...
                # For binary classification, flip to opposite class
                y_target = 1 - y
...
def save_results(results: dict, args, timestamp: str, logger: logging.Logger):
    ...
    # Save to CSV
    csv_path = output_dir / f'adult_evaluation_{timestamp}.csv'
    df = pd.DataFrame([results])
    df.to_csv(csv_path, index=False)
    logger.info(f"Results saved to CSV: {csv_path.absolute()}")

    # Save to JSON with metadata
    json_path = output_dir / f'adult_evaluation_{timestamp}.json'
    full_results = {
        'timestamp': timestamp,
...
def save_statistics(
    ...
    """Save detailed statistics about counterfactuals.

    Args:
        originals: Original samples [N, 108]
        counterfactuals: Generated counterfactuals [N, 108]
    ...
    diff = (counterfactuals - originals).abs()

    stats = {
        'mean_change_per_feature': diff.mean(dim=0).numpy().tolist(),
        'std_change_per_feature': diff.std(dim=0).numpy().tolist(),
        'max_change_per_feature': diff.max(dim=0)[0].numpy().tolist(),
        'num_changed_features': (diff > 0.01).float().sum(dim=1).mean().item(),
        'total_l1_distance': diff.sum(dim=1).mean().item(),
    }
    ...
    for target_class in [0, 1]:
        ...
        if mask.sum() > 0:
            stats[f'count_target_{target_class}'] = mask.sum().item()
            stats[f'mean_l1_target_{target_class}'] = diff[mask].sum(dim=1).mean().item()

    # Save to JSON
    stats_path = output_dir / f'adult_statistics_{timestamp}.json'
    with open(stats_path, 'w') as f:
        json.dump(stats, f, indent=2)

    logger.info(f"Statistics saved to: {stats_path.absolute()}")
    logger.info(f"Mean number of changed features: {stats['num_changed_features']:.2f}")
...
def print_summary(results: dict, logger: logging.Logger):
    ...
    """
    logger.info("\n" + "="*60)
    logger.info("EVALUATION SUMMARY - ADULT CF-TCVAE")
    logger.info("="*60)
...
def main():
    ...
    # Setup logging
    logger = setup_logging('evaluate_adult', timestamp)
    ...
    logger.info("="*60)
    logger.info("CF-TCVAE ADULT EVALUATION")
    logger.info("="*60)
    ...
    logger.info(f"Target class: {args.target_class or 'flip to opposite'}")
    logger.info("="*60 + "\n")

    # Load datasets
    logger.info("Loading Adult dataset...")
    train_loader, val_loader, test_loader = create_adult_loaders(
        batch_size=args.batch_size,
    ...
    classifier = load_adult_classifier(device=args.device)
    ...
    if args.save_statistics:
        logger.info("Computing and saving detailed statistics...")
        save_statistics(
            originals=originals,
            counterfactuals=counterfactuals,
            original_labels=original_labels,
