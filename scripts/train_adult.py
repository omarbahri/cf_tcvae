#!/usr/bin/env python3
"""Training script for CF-TCVAE on Adult Income dataset.

Single-phase unified training with all loss components from Equation 15.
NO phase1/phase2 logic.

Usage:
    python scripts/train_adult.py --max_epochs 200 --batch_size 1024 --beta 10.0
    python scripts/train_adult.py --config configs/adult_default.yaml
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train CF-TCVAE on Adult Income')
    ...
    parser.add_argument('--alpha_proximity', type=float, default=2.0,
    ...
    # Prior
    parser.add_argument('--prior_ema_momentum', type=float, default=0.001,
                        help='EMA momentum for prior updates')

    # Directories
    parser.add_argument('--checkpoint_dir', type=str, default='checkpoints/adult',
...
def main():
    ...
    logger = setup_logging('train_adult', timestamp)

    logger.info("=" * 80)
    logger.info("CF-TCVAE Training on Adult Income")
    logger.info("=" * 80)

    # Load configuration
    if args.config is not None:
        logger.info(f"Loading config from: {args.config}")
        config = AdultConfig.from_yaml(args.config)
    else:
        logger.info("Using default Adult config with CLI overrides")
        config = AdultConfig()
    ...
    logger.info("Loading pre-trained Adult classifier...")
    try:
        classifier = load_adult_classifier(device=device)
        logger.info("Classifier loaded and frozen")
    except Exception as e:
        logger.error(f"Failed to load Adult classifier: {e}")
        logger.error("Note: Adult classifier checkpoint may need to be retrained in standalone environment")
        logger.error("See plan_progress.md Step 4 for details")
        raise
